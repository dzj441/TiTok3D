...
class Decoder(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # middle
        for block in self.mid: # same in same out
            hidden_states = block(hidden_states)
        # [1,512,16,16]
        # up-sampling
        for block in reversed(self.up):
            hidden_states = block(hidden_states) # [1,512,32,32] [1,256,64,64]  [1,256,32,32] [1,128,256,256]

        # end
        hidden_states = self.norm_out(hidden_states)
        hidden_states = F.silu(hidden_states)
        hidden_states = self.conv_out(hidden_states) # [1,3,256,256]
