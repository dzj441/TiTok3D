...
class VectorQuantizer(torch.nn.Module):
    ...
    def forward(self, z: torch.Tensor) -> Tuple[torch.Tensor, Mapping[Text, torch.Tensor]]:
        ...
        d = torch.sum(z_flattened**2, dim=1, keepdim=True) + \
            ...
            torch.einsum('bd,dn->bn', z_flattened, embedding.T) # look up : distance on each of the 4096 entries(dim 12)

        min_encoding_indices = torch.argmin(d, dim=1) # num_ele
        z_quantized = self.get_codebook_entry(min_encoding_indices).view(z.shape) # [1,1,32,12]
        ...
        result_dict = dict(
            ...
            min_encoding_indices=min_encoding_indices.view(z_quantized.shape[0], z_quantized.shape[2], z_quantized.shape[3]) # [1,1,32] selected entries idx
        )
